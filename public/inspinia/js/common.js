$( function(){
			...
			$.ajax( {
				...
				success: function( msg ){
					...
					if( res.error == 'ok' ){
						...
						$( "#icon-preview" ).removeAttr( 'data-icon' );
	...
	$( "body" ).on( "keyup", ".top-search", function(){
		...
		if( $( '#searchResLoad' ).length === 0 ){
			$( '.ibox-content' ).prepend( '<div id="searchResLoad"></div>' );
		}
		...
		if( search.length > 3 ){
			...
			$( '#searchResLoad' ).load( "/search_for_admin",
				{
				search: search,
				_token: $( "#_token" ).val()
			},function(msg  ){
