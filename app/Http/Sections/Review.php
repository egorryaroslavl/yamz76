...
class Review extends Section implements Initializable
{
	...
	public function onDisplay()
	{
		return AdminDisplay::datatables()
			...
			->setHtmlAttribute( 'class', 'table-reviews' )
			->setHtmlAttribute( 'data-table', 'reviews' )
			->setTitle( 'Отзывы' )
			->setColumns(
				AdminColumn::custom()->setLabel( 'Reorder' )
					->setCallback(
						function (){
							return '<input type="hidden" class="table"  name="table" value="reviews">';

						}

					)->setWidth( '30px' ),

				AdminColumn::link( 'name', 'Отзыв' )->setWidth( '50%' ),
				AdminColumn::custom()
					->setLabel( 'Публикуется' )
					->setCallback( function ( \App\Model\Review $model ){
